    ...
    >
      ...
      <nav className="w-full py-2 md:pb-4 pb-1 border-b-2 text-white">
        <div className="flex justify-between md:justify-center gap-8 items-center px-4 py-3 md:py-4 md:px-6">
          ...
          <div className="block md:hidden text-black text-center text-md font-black">
        ...
        >
          ...
          <div
            className={`flex flex-col md:flex-row md:gap-4 gap-2 py-2 md:py-0 ${i18n.language === "ar" ? "md:flex-row-reverse" : "md:flex-row"
              }`}
          >
            {menuItems.map((item) => (
              <Link
                key={item.id}
                to={item.path}
                className={`px-3 py-2 rounded-md text-sm font-black md:text-base ${selectedTitle === item.title
