  ...
  return (
		...
		>
			...
			<nav className="w-full py-2 md:pb-4 pb-1 border-b-2 text-white">
				<div
					className={`md:flex md:justify-center md:items-center md:p-0 bg-gray-100 md:bg-transparent ${
						isMenuOpen ? "block" : "hidden"
					}`}
				>
					<div
						className={`flex flex-col md:flex-row md:gap-4 py-2 md:py-0 ${
						...
						{menuItems.map((item) => (
							...
							>
								<div className="flex items-center w-full">
									<Link
										to={item.path}
										className={`flex-grow block px-4 py-2 md:px-3 md:py-2 text-sm font-black md:text-base md:rounded-md ${
										...
										<span className="flex items-center justify-between w-full">
											...
											{item.submenu && (
												<>
													...
													<ChevronRight
														className={`md:hidden w-4 h-4 transition-transform duration-200 ${
								...
								{item.submenu && openSubmenu === item.id && (
									<div
										className={`w-full md:absolute md:left-0 md:top-full md:w-48 bg-white ${
